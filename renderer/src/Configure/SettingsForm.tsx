import { ActionIcon, Button, Container, Grid, Group, InputWrapper, NumberInput, Text, TextInput } from '@mantine/core';
...
const SettingsForm = (props: Props) => {
	const form = useForm({
		initialValues: {
			workDuration: parseInt(window.localStorage.getItem('workDuration') || '50'),
			...
			shortBreakLabel: window.localStorage.getItem('shortBreakLabel') || 'Short break',
			longBreakLabel: window.localStorage.getItem('longBreakLabel') || 'Long break',
	...
	const handleFormSubmit = (ev: any) => {
		const { workDuration, shortBreak, longBreak, workLabel, shortBreakLabel, longBreakLabel, fileDirectory } =
			form.values;
		ev.preventDefault();
	...
	return (
		<form onSubmit={(ev) => handleFormSubmit(ev)}>
			...
			<Grid gutter='md'>
				<Grid.Col span={4}>
					<TextInput
						...
						onChange={(event) => form.setFieldValue('workLabel', event.target.value)}
						min={1}
						label='Work label'
					/>
				</Grid.Col>
				<Grid.Col span={4}>
					<TextInput
						...
						onChange={(event) => form.setFieldValue('shortBreakLabel', event.target.value)}
						min={1}
				...
				<Grid.Col span={4}>
					<TextInput
						...
						onChange={(event) => form.setFieldValue('longBreakLabel', event.target.value)}
						min={1}
			...
			{form.values.fileDirectory && <Text size='sm'>{form.values.fileDirectory}</Text>}
